...
const _logDecl = (decl) => `${decl.prop}: ${decl.value}`
const _logDeclParent = (decl, isUnderAtRule) => isUnderAtRule ? `@media(${decl.parent.parent.params}) ${decl.parent.selector}` : `${decl.parent.selector}`
const _logDep = ({ prop, calculatedInPlaceValue, decl, isUnderAtRule }) => `${_logDeclParent(decl, isUnderAtRule)} ${prop} = ${calculatedInPlaceValue}`
	...
	variablesUsedList.forEach(function(variableUsedName) {
		...
		gatherVariableDependencies(variablesUsedList, map).deps.forEach(function(mapItem) {
			...
			if(mapItem.isUnderAtRule) {
				...
				console.log(chalk.cyan('\n[[@atrule]]'), `Resolving: ${_logDep(mapItem)}`)
				...
				if (mapItem.isRootSpecific) {
					console.log(chalk.red("Need another scope insertion"))
					...
					console.log('mimicDecl selector', chalk.green(mimicDecl.parent.selector))
					console.log('new scoped', generateScopeList(mimicDecl.parent, true))
					console.log('scope of the parent', generateScopeList(mapItem.parent, true))
					console.log(chalk.red("Scope should be inserted"))
				}
				...
				console.log('current declaration', generateScopeList(decl.parent, true));
				console.log('new scoped', generateScopeList(mimicDecl.parent, true));
				console.log('scope of variable', generateScopeList(mapItem.parent, true));
				...
				console.log(chalk.yellow('\n[[:root]]'), `Resolving: ${_logDep(mapItem)}`, mapItem.parent.selectors)
				...
				console.log('current declaration', generateScopeList(decl.parent, true))
				console.log('mimicDecl selector', mimicDecl.parent.selector)
				console.log('new scoped', generateScopeList(mimicDecl.parent, true))
				console.log('scope of variable', generateScopeList(mapItem.parent, true))
			}
			...
			if(mimicDecl && isNodeUnderScope(mimicDecl, mapItem.parent, true)) {
				...
				console.log('FOR-EVERY-DEP', _logDep(mapItem), 'has no generated content')
			}
		});
	});
}
...
function resolveDecl(decl, map, /*optional*/shouldPreserve, /*optional*/preserveAtRulesOrder, /*optional*/logResolveValueResult) {
	console.log(chalk.cyan(`\n\n\n--- RESOLVING DECLARATION ${decl.parent.selector}: ${_logDecl(decl)} ---`))
